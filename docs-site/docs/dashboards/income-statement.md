...
The Income Statement is generated by querying the `Income` and `Expenses` subtrees:

### Market Value Query
```sql
SELECT account, convert(sum(position), 'USD') WHERE account ~ '^(Income|Expenses)' AND NOT close_date(account) GROUP BY account ORDER BY account
```

### At Cost Query
```sql
SELECT account, cost(sum(position)) WHERE account ~ '^(Income|Expenses)' AND NOT close_date(account) GROUP BY account ORDER BY account
```

### Units Query
```sql
SELECT account, units(sum(position)) WHERE account ~ '^(Income|Expenses)' AND NOT close_date(account) GROUP BY account ORDER BY account
```
